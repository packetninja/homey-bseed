...
class HybridSwitchBase extends ZigBeeDevice {
  ...
  async onNodeInit({ zclNode }) {
    ...
    this.log(`Hybrid ${typeConfig.name} ready: ${this._protocolInfo.modelId} (${this.gangCount} gang, ${this._protocolInfo.protocol})`);

    await this._migrateCapabilities();
    this._bumpMaxListeners(zclNode);

    // Try Tuya magic spell for multi-gang devices (TS0002/TS0003 support)
    if (this.gangCount >= 2) {
      await this._tryTuyaMagicSpell(zclNode).catch(err => {
        this.log('[MAGIC-SPELL] Could not apply (this is OK):', err.message);
      });
    }

    // v5.5.63: Initialize Protocol Auto-Optimizer
    this.protocolOptimizer = new ProtocolAutoOptimizer(this, { verbose: false });
    await this.protocolOptimizer.initialize(zclNode);

    this.protocolOptimizer.on('decision', (mode) => {
      this.log(`Protocol: ${mode}`);
    });
  ...
  /**
   * Try to send Tuya "magic spell" for multi-gang devices (TS0002/TS0003)
   *
   * This attempts to fix the mirroring bug where all gangs turn on/off together.
   * The magic spell activates multi-endpoint independent operation mode.
   *
   * Works for devices that have Tuya attributes in Basic cluster (0xFFDE, 0xFFFE)
   * even if they don't have the full manuSpecificTuya cluster (0xEF00).
   *
   * Based on Zigbee2MQTT fix: https://github.com/Koenkk/zigbee-herdsman-converters/pull/3335
   */
  async _tryTuyaMagicSpell(zclNode) {
    const endpoint = zclNode?.endpoints?.[1];
    if (!endpoint?.clusters?.basic) {
      this.log('[MAGIC-SPELL] No Basic cluster found');
      return;
    }

    this.log('[MAGIC-SPELL] 🔍 Checking for Tuya attributes...');

    try {
      // Step 1: Check if device has Tuya magic spell attributes
      const basicCluster = endpoint.clusters.basic;

      // Try to read Tuya capability attribute (0xFFFE / 65534)
      let hasTuyaAttributes = false;
      try {
        await basicCluster.readAttributes([0xFFFE]);
        hasTuyaAttributes = true;
        this.log('[MAGIC-SPELL] ✅ Device has Tuya capability attribute (0xFFFE)');
      } catch (err) {
        this.log('[MAGIC-SPELL] ⚠️ No Tuya capability attribute (0xFFFE) - device likely pure ZCL');
        this.log('[MAGIC-SPELL] This is OK - magic spell not needed for this device');
        return;
      }

      if (!hasTuyaAttributes) {
        return;
      }

      // Step 2: Send the magic spell
      this.log('[MAGIC-SPELL] 🎯 Sending magic spell to activate multi-endpoint mode...');

      // Read basic attributes including Tuya-specific one (signals Tuya feature support)
      try {
        await basicCluster.readAttributes([
          ...
          'modelId',
          'powerSource',
          0xFFFE  // Tuya-specific capability attribute
        ]);
        this.log('[MAGIC-SPELL] ✅ Read Tuya attributes');
      } catch (err) {
        this.log('[MAGIC-SPELL] ⚠️ Could not read Tuya attributes:', err.message);
      }

      // Write magic value to activate multi-endpoint mode
      // 0xFFDE (65502) = operating mode selector
      // Value 19 = multi-endpoint independent operation mode
      try {
        await basicCluster.writeAttributes({
          [0xFFDE]: 19
        });
        this.log('[MAGIC-SPELL] ✅ Wrote magic value (0xFFDE = 19) - Multi-endpoint mode activated!');
      } catch (err) {
        this.log('[MAGIC-SPELL] ⚠️ Could not write magic value:', err.message);
        // Continue anyway - read might have been enough
      }

      // Step 3: If device has manuSpecificTuya cluster, send dataQuery
      ...
      if (tuyaCluster && typeof tuyaCluster.dataQuery === 'function') {
        try {
          ...
          this.log('[MAGIC-SPELL] ✅ Sent dataQuery command');
        } catch (err) {
          this.log('[MAGIC-SPELL] ⚠️ Could not send dataQuery:', err.message);
        }
      } else {
        this.log('[MAGIC-SPELL] ℹ️ No Tuya cluster for dataQuery (ZCL mode, this is OK)');
      }

      // Log firmware version for documentation
      try {
        const fwInfo = await basicCluster.readAttributes(['appVersion', 'hwVersion']);
        this.log('[MAGIC-SPELL] 📊 Firmware: app=' + fwInfo.appVersion + ', hw=' + fwInfo.hwVersion);

        // Store for easy access
        await this.setSettings({
          firmware_app_version: String(fwInfo.appVersion || 'unknown'),
          firmware_hw_version: String(fwInfo.hwVersion || 'unknown')
        }).catch(() => {});
      } catch (err) {
        // Ignore
      }

      this.log('[MAGIC-SPELL] ✅ Magic spell sequence complete!');
      this.log('[MAGIC-SPELL] ℹ️ Device should now operate gangs independently');
      this.log('[MAGIC-SPELL] ℹ️ Test: Turn on gang 1 - gang 2 should stay off');

    } catch (err) {
      this.log('[MAGIC-SPELL] ❌ Error during magic spell:', err.message);
